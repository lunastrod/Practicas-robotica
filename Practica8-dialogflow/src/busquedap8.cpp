...
public:
  ...
  bool service_function(servicios::busqueda::Request &req, servicios::busqueda::Response &res)
  {
    int centrox;
    int centroy;
    if(!boxes_darknet.bounding_boxes[0].Class.compare(req.object.data)){
      centrox = (boxes_darknet.bounding_boxes[0].xmin + boxes_darknet.bounding_boxes[0].xmax) / 2;
      centroy = (boxes_darknet.bounding_boxes[0].ymin + boxes_darknet.bounding_boxes[0].ymax) / 2;
    }
